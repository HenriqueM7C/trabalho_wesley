...
app = Flask(__name__)
# cria uma instância da classe Api do Flask-RESTx, vinculando-a à instância do Flask representada por app
...
folha_pagamento_model = api.model('Folha de Pagamento', {
    #fields.Integer é usado para indicar que o valor desse campo deve ser um número inteiro.
    #readonly=True especifica que este campo é apenas para leitura,
    #fields.String é usado para indicar que o valor desse campo deve ser uma string.
    #required=True especifica que este campo é obrigatório ao enviar dados para a API.
...
class FolhaPagamentoResource(Resource):
    ...
    @api.expect(parser)
    # especifica o modelo de resposta que deve ser aplicado à saída do método decorado.
